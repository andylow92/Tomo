...
const PinchSvg = () => (
  <svg viewBox="0 0 128 128" overflow="visible">
    ...
    <g className="body-group">
      <rect
        x="34" y="28" width="60" height="75" rx="30"
        fill="#FF8C69" stroke="#8B3A3A" strokeWidth="4"
      />
      {/* Belt */}
      <path
        d="M 34 82 Q 64 92 94 82 L 94 88 Q 64 98 34 88 Z"
        fill="#008080" stroke="#004D4D" strokeWidth="2"
      />
      {/* Belt buckle */}
      <path
        d="M 75 85 L 88 108 L 68 102 Z"
        fill="#008080" stroke="#004D4D" strokeWidth="2" strokeLinejoin="round"
      />
      {/* Face */}
      <g className="face">
        {/* Eyes Open */}
...
const ShibaSvg = () => (
  <svg viewBox="0 0 128 128" overflow="visible">
    ...
    <g className="tail">
      <path
        d="M 56 108 Q 42 85 30 90 Q 25 92 28 96 Q 35 102 50 108"
        fill="#E8A735"
        stroke="#B07A1A"
        strokeWidth="2.5"
        strokeLinecap="round"
    ...
    <g className="head-group">
      ...
      {/* Cheek mask */}
      <path
        d="M 42 52 Q 48 42 64 40 Q 80 42 86 52 Q 86 68 64 72 Q 42 68 42 52"
        fill="#F5E6C8"
      />
    ...
    <g className="head-group">
      ...
      <g className="face">
        ...
        <g className="mouth-worried">
          <rect x="54" y="48" width="20" height="3" rx="1.5" fill="#FF5252" opacity="0.8" transform="rotate(0 64 49)" />
...
const MonkeySvg = () => (
  <svg viewBox="0 0 128 128" overflow="visible">
    {/* Tail - long curling tail */}
    <g className="tail">
      <path
        d="M 48 100 Q 20 95 15 75 Q 12 60 20 50 Q 25 44 28 48"
        fill="none"
        stroke="#8B5E3C"
        strokeWidth="5"
        strokeLinecap="round"
...
.pet-container .fx-banana { opacity: 0; transition: opacity 0.3s; }
...
/* ═══════════════════════════════════════════════
   PINCH (LOBSTER) SPECIFIC ANIMATIONS
   ═══════════════════════════════════════════════ */
...
.pinch-container.state-curious .body-group { animation: none; }
...
@keyframes snipSnip { 0%,100% { transform: rotate(0deg); } 25% { transform: rotate(-25deg) scale(1.1); } 50% { transform: rotate(0deg); } 75% { transform: rotate(-25deg) scale(1.1); } }
...
.shiba-container.state-happy .eye-open { animation: none; }
.shiba-container.state-happy .eye-closed { opacity: 1; }
.shiba-container.state-happy .eye-open { opacity: 0; }
...
.robot-container.state-happy .eye-open .left-eye-led,
.robot-container.state-happy .eye-open .right-eye-led { fill: #66BB6A; }
...
.robot-container.state-sleeping .antenna-glow { opacity: 0.3; animation: pulse 4s ease-in-out infinite; }
.robot-container.state-sleeping .mouth-neutral { opacity: 0; }
.robot-container.state-sleeping .mouth-happy { opacity: 0; }
...
.robot-container.state-curious .eye-open .left-eye-led,
.robot-container.state-curious .eye-open .right-eye-led { fill: #FDD835; }
...
.robot-container.state-worried .antenna-ball { fill: #FF5252; }
.robot-container.state-worried .antenna-glow { fill: #FF8A80; animation: pulse 0.3s ease-in-out infinite; }
.robot-container.state-worried .eye-open .left-eye-led,
.robot-container.state-worried .eye-open .right-eye-led { fill: #FF5252; }
...
.robot-container.state-proud .eye-open .left-eye-led,
.robot-container.state-proud .eye-open .right-eye-led { fill: #AB47BC; }
...
.robot-container.state-startle .antenna-ball { fill: #FF5252; }
.robot-container.state-startle .antenna-glow { fill: #FF8A80; animation: pulse 0.2s ease-in-out infinite; }
...
.robot-container.state-startle .eye-open .left-eye-led,
.robot-container.state-startle .eye-open .right-eye-led { fill: #FF5252; animation: pulse 0.2s ease-in-out infinite; }
...
.robot-container.state-mad .antenna-ball { fill: #FF5252; }
...
.monkey-container.state-happy .left-arm { transform: translateY(-22px) rotate(-45deg); animation: monkeyClap 0.35s ease-in-out infinite; }
.monkey-container.state-happy .right-arm { transform: translateY(-22px) rotate(45deg); animation: monkeyClapR 0.35s ease-in-out infinite; }
...
.monkey-container.state-curious .body-group { animation: none; }
...
.monkey-container.state-worried .left-ear { transform: rotate(8deg); }
.monkey-container.state-worried .right-ear { transform: rotate(-8deg); }
...
.monkey-container.state-proud .tail { animation: tailSwing 1s ease-in-out infinite; transform: rotate(-15deg); }
...
.monkey-container.state-startle .left-ear { transform: translateY(-3px) scale(1.1); }
.monkey-container.state-startle .right-ear { transform: translateY(-3px) scale(1.1); }
...
export default function App() {
  ...
  }, [pinchMood, shibaMood, robotMood, monkeyMood, triggerIdle]);

  const setMoodForBoth = (m) => {
    if (pinchTimer.current) clearTimeout(pinchTimer.current);
    if (shibaTimer.current) clearTimeout(shibaTimer.current);
      ...
      <div className="app">
        ...
        <div className="pets-row">
          <PetDisplay
            name="Pinch"
            label="Lobster"
            SvgComponent={PinchSvg}
            type="pinch"
            mood={pinchMood}
            idleBehavior={pinchIdle}
            onPoke={() => pokePet(setPinchMood, setPinchIdle, pinchTimer)}
            accent="#FF8C69"
          />
          <PetDisplay
            name="Mochi"
            label="Shiba Inu"
            SvgComponent={ShibaSvg}
            type="shiba"
            mood={shibaMood}
            idleBehavior={shibaIdle}
            onPoke={() => pokePet(setShibaMood, setShibaIdle, shibaTimer)}
            accent="#E8A735"
          />
          <PetDisplay
            name="Unit-7"
            label="Robot"
            SvgComponent={RobotSvg}
            type="robot"
            mood={robotMood}
            idleBehavior={robotIdle}
            onPoke={() => pokePet(setRobotMood, setRobotIdle, robotTimer)}
            accent="#4FC3F7"
          />
          <PetDisplay
            name="Coco"
            label="Monkey"
            SvgComponent={MonkeySvg}
            type="monkey"
            mood={monkeyMood}
            idleBehavior={monkeyIdle}
            onPoke={() => pokePet(setMonkeyMood, setMonkeyIdle, monkeyTimer)}
            accent="#C4895E"
          />
        </div>
        ...
        <div className="controls-section">
          ...
          <div className="controls">
            {MOODS.map((m) => (
              <button
                key={m}
                className={`mood-btn ${pinchMood === m && shibaMood === m && robotMood === m && monkeyMood === m ? "active" : ""}`}
                onClick={() => setMoodForBoth(m)}
              >
                {m}
              </button>
            ))}
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setTimeout(() => {
                    ...
                    setMonkeyIdle(b);
                    const durations = { blink: 400, look_around: 2000, stretch: 2000, wiggle: 1200, yawn: 2500 };
                    setTimeout(() => { setPinchIdle(null); setShibaIdle(null); setRobotIdle(null); setMonkeyIdle(null); }, durations[b] || 1500);
                  }, 100);
                }}
              >
                {b.replace("_", " ")}
